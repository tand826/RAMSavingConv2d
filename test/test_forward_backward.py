...
def forward_backward(dtype):
    image, label, net, optimizer, criterion = prepare(mode="default", dtype=dtype, skip_input_grad=False)
    normal_conv = {}
    optimizer.zero_grad()
    outputs = net(image)
    ...
    image, label, net, optimizer, criterion = prepare(mode="ramsaving", dtype=dtype, skip_input_grad=False)
    cs_conv = {}
    optimizer.zero_grad()
    outputs = net(image)
