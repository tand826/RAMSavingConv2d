...
def test_fp16():
    forward_backward_no_inputgrad(dtype=torch.float16)


def test_fp32():
    forward_backward_no_inputgrad(dtype=torch.float32)


def forward_backward_no_inputgrad(dtype):
    image, label, net, optimizer, criterion = prepare(mode="default", dtype=dtype, skip_input_grad=True)
    normal_conv = {}
    optimizer.zero_grad()
    outputs = net(image)
    ...
    image, label, net, optimizer, criterion = prepare(mode="ramsaving", dtype=dtype, skip_input_grad=True)
    cs_conv = {}
    optimizer.zero_grad()
    outputs = net(image)
